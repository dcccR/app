...
server <- function(input, output) {
  observeEvent(input$file, {
    ...
    river <- unique(data$河川代碼)
    
    # 定義 river_function
    river_function <- function(i) {
      data_river <- data[data$河川代碼 == river[i], ]
      n_sites <- length(unique(data_river$locality))
      n_species <- length(unique(data_river$scientificName))
      
      row_names <- unique(data_river$locality)
      col_names <- unique(data_river$scientificName)
      
      data_matrix <- matrix(NA, nrow = n_sites, ncol = n_species) 
      row.names(data_matrix) <- row_names
      ...
      for (i in 1:length(row_names)) {
        for (j in 1:length(col_names)) {
          ...
          if (z > 0) {
            data_matrix[row_names[i], col_names[j]] <- 1
          } else {
            data_matrix[row_names[i], col_names[j]] <- 0
          }
        }
      }
      
      data_matrix <- t(data_matrix)
      all_site_num <- colSums(data_matrix)
      able_site <- which(all_site_num != 0)
      clean_data <- data_matrix[, able_site]
    ...
    withProgress(message = 'Processing data...', value = 0, {
      ...
      for (i in 1:n) {
        ...
        incProgress(1/n, detail = paste("Processing river", i, "of", n))
      }
    })
    ...
    observeEvent(input$calculate,{
      ...
      output$good <- renderPrint({
        if (!is.na(input$c) && !is.na(input$t)) {
          ...
          site_selection_result <- function(x, t, n = 100) {
          ...
          result <- list(colnames(a[[which.min(k)]]), c(good_site, species_num))
          
          z <- data[data$河川代碼 == input$c, ]
          
          q <- data.frame(locality = colnames(a[[which.min(k)]]))
          ...
          output$plot <- renderPlot({
            ggmap(get_googlemap(center = c(lon = lon_part, lat = lat_part), zoom = 10, maptype = 'terrain')) +
              geom_point(data = z, aes(x = decimalLongitude, y = decimalLatitude), color = "blue", size = 2) +
              geom_point(data = q, aes(x = decimalLongitude, y = decimalLatitude), color = "red", size = 3)
